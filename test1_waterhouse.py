...
cv2.imwrite('results/blurM.png', blurM)

# apply gaussian filter for smoothing
blurG = cv2.GaussianBlur(gray, (9, 9), 0)
cv2.imwrite('results/blurG.png', blurG)

# histogram equalization
histoNorm = cv2.equalizeHist(gray)
cv2.imwrite('results/histoNorm.png', histoNorm)

# create a CLAHE object for
# Contrast Limited Adaptive Histogram Equalization (CLAHE)
clahe = cv2.createCLAHE(clipLimit=2.0, tileGridSize=(8, 8))
claheNorm = clahe.apply(gray)
cv2.imwrite('results/claheNorm.png', claheNorm)
...
cv2.imwrite('results/contrast_stretch.png', contrast_stretched)
cv2.imwrite('results/contrast_stretch_blurM.png',
...
cv2.imwrite('results/edgeG.png', edgeG)

edgeM = cv2.Canny(blurM, 100, 200)
cv2.imwrite('results/edgeM.png', edgeM)

# read enhanced image
img = cv2.imread('results/contrast_stretch.png', 0)
...
display = cv2.imread("5a-50lum-10x-365-2.tif")
