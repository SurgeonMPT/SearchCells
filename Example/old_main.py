...
if __name__ == '__main__':
    # folder = 'K/ht-29/'
    # fname = 'K/ht-29/5a-50lum-10x-365-2'
    # ftype = '.tif'
    # file = folder + fname + ftype
    file = 'scan/ht-29/5a-50lum-10x-365-2.tif'
    image_grayscale = cv2.imread(file, cv2.IMREAD_GRAYSCALE)
    image_rgb = cv2.cvtColor(image_grayscale, cv2.COLOR_GRAY2RGB)
    contour_set = adaptive_thresholding(file, blurr=5)
    # contour_set = optimal(file)

    # for contour_num in range(len(contour_set)):
    #     save_contour(image_grayscale, contour_set, contour_num)
    print('done')
    ...
    #     print_image(selected_print, f'contour #{contour_num}')
    #
    # selected = select_contour(contour_num=1, image_rgb=image_rgb, contours_set= contour_set, print_result = True)

    # image_grayscale = cv2.imread(file, cv2.IMREAD_GRAYSCALE)
    # image_rgb = cv2.cvtColor(image_grayscale, cv2.COLOR_GRAY2RGB)
    #
    # contours = adaptive_thresholding(file, blurr= 5)
    #
    # cnt = 27
    # selected = select_contour(contour_num=cnt, image_grayscale=image_grayscale, contours_set=contours, print_result=True)
    # #print_image(selected,"selected")
    #
    # ret, mask = cv2.threshold(selected, 30, 255, cv2.THRESH_BINARY)
    # masked_image = cv2.bitwise_and(selected, mask)
    #
    # canny = cv2.Canny(masked_image, 50, 90, 0)
    # dilated = cv2.dilate(canny, (2, 2), iterations=3)
    ...
    # for cnt in contour_filtered_by_length:
    #     cv2.fillPoly(stencil, contour_filtered_by_length, color)
    # result = cv2.bitwise_and(image_rgb.copy(), stencil)
    # cv2.drawContours(result, contour_filtered_by_length, -1, (255, 0, 0), 2)
    # print_image(result, "new")
