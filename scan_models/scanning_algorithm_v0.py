...
class ScanningAlgorithmV0:
    ...
    def print_contours(self, image_rgb, contours_set):
        color = [255, 255, 255]
    ...
    def cell_counting_image(self, image_, noise_treshold=5, threshold_low=10, threshold_top=255, minimum_length=1, blur=1,
    ...
    def scan(self, file):
        ...
        print(f'Testing {ScanningAlgorithmV0.NAME} method')

        image_grayscale = cv2.imread(file, cv2.IMREAD_GRAYSCALE)
        image_rgb = cv2.cvtColor(image_grayscale, cv2.COLOR_GRAY2RGB)
        contour_set = self.adaptive_thresholding(file, blurr=5)

        plt.savefig('result.png')
        return {
            'method_name': ScanningAlgorithmV0.NAME,
